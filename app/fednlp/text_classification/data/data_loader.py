...
def load_synthetic_data(args):
    ...
    elif args.formulation == 'seq_tagging':
        model_args = SeqTaggingArgs()
    elif args.formulation == "span_extraction":
        model_args = SpanExtractionArgs()
    elif args.formulation == 'seq2seq':
    ...
    if args.formulation != 'seq2seq':
        tokenizer = tokenizer_class.from_pretrained(args.model, do_lower_case=args.do_lower_case)
    else:
        tokenizer = [None, None]
        tokenizer[0] = tokenizer_class.from_pretrained(args.model)
        tokenizer[1]= tokenizer[0]
    if args.formulation == 'classification':
        preprocessor = TCPreprocessor(args=model_args, label_vocab=attributes["label_vocab"],tokenizer=tokenizer)
        dm = TextClassificationDataManager(args, model_args, preprocessor, 0, args.client_num_per_round)
    elif args.formulation == "seq_tagging":
        preprocessor = STPreprocessor(args=model_args, label_vocab=attributes["label_vocab"],tokenizer=tokenizer)
        dm = SequenceTaggingDataManager(args, model_args, preprocessor, 0, num_workers)
    elif args.formulation == "span_extraction":
        ...
        dm = Seq2SeqDataManager(args, model_args, preprocessor, 0, args.client_num_per_round)

    train_data_num, test_data_num, train_data_global, test_data_global, train_data_local_num_dict, train_data_local_dict, test_data_local_dict, num_clients = dm.load_federated_data()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_class', type = str, default='transformer', metavar='N', help='model class')

    parser.add_argument('--model', type=str, default='bert-base-uncased', metavar='N',
    ...
    parser.add_argument('--clip_grad_norm', type=int, default=0, metavar='N',
                        help='L2 penalty')
    ...
    parser.add_argument(
        '--evaluate_during_training', default=False, metavar='EP',
