        ...
        if len(self.X) == 0 or len(self.Y) == 0 or self.attributes["label_vocab"] is None:
            ...
            for root, _, files in os.walk(self.data_path):
                for file_name in files:
                    ...
                    elif file_name.endswith("train.txt") or file_name.endswith("dev.txt"):
                        size = self.process_data_file(file_path)
                        train_size += size
            ...
            assert len(self.attributes["train_index_list"]) == len(self.attributes["train_source_list"])
            assert len(self.attributes["test_index_list"]) == len(self.attributes["test_source_list"])
            assert len(self.attributes["index_list"]) == len(self.attributes["source_list"])
