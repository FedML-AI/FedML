...
def build(source_folder, entry_point, config_folder, dest_folder, ignore,
    ...
    if model_cache_path is not None and str(model_cache_path).strip() != "":
        config_dict["model_cache_path"] = model_cache_path
    ...
    if output_dim is not None and str(output_dim).strip() != "":
        config_dict["output_dim"] = output_dim
    ...
    build(ModuleConstants.PLATFORM_NAME_LAUNCH, ModuleConstants.TRAIN_BUILD_PACKAGE_CLIENT_TYPE,
