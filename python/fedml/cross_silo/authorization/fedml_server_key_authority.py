...
from ..server.message_define import MyMessage
...
class FedMLServerKeyAuthority(FedMLCommManager):
    def __init__(
        ...
        self.fhe_batch = args.batch_size
        self.fhe_scale = args.scaling_factor
        self.fhe_file_dir = args.file_loc

    def run(self):
        super().run()

    def generate_key(self):
        #Key authority server generates cryptocontext and key pair (a public key and a private key)
        #using crypto parameters specified in the args
        self.fhe_helper = fhe_core.CKKS('ckks', self.fhe_batch,  self.fhe_scale, self.fhe_file_dir)
        # after generation, there are three files (1) cryptocontext.txt (sent to all both the server and clients);
        # (2) key-public.txt (sent to all both the server and clients); (3) key-private (sent to only clients).
        self.fhe_helper.genCryptoContextAndKeyGen()
