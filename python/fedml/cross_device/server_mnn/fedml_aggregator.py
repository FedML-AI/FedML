...
class FedMLAggregator(object):
    def __init__(
        ...
        """
        Initialize the FedMLAggregator.

        Args:
            test_dataloader: DataLoader for the test dataset.
            worker_num: Number of worker nodes (clients).
            device: The device (e.g., CPU or GPU) to use for computations.
            args: Arguments for configuration.
            aggregator: The aggregator used for federated learning aggregation.
    ...
    def get_global_model_params(self):
        """
        Test the global model on the server using the MNN (Mobile Neural Network) file format.

        Args:
            mnn_file_path: The path to the MNN file containing the global model.
            round_idx: The current round index.
            report_metrics: A boolean indicating whether to report metrics (default is True).
    ...
    def get_global_model_params_file(self):
        """
        Get the file path of the global model parameters.

        Returns:
            str: File path of the global model parameters.
        """
    ...
    def set_global_model_params(self, model_parameters):
        ...
        Args:
            model_parameters: Parameters of the global model.

        Returns:
            None
        """
        logging.info(
            "FedDebug. model_parameters = {}".format(model_parameters))
        self.aggregator.set_model_params(model_parameters)

    def add_local_trained_result(self, index, model_params, sample_num):
        """
        Add the results of local model training for aggregation.

        Args:
            index (int): Index of the local client.
            model_params: Parameters of the locally trained model.
            sample_num (int): Number of samples used for training.
    ...
    def _test_individual_model_perf_before_agg(self, model_file_path, round_idx):
        """
        Test the performance of an individual model before aggregation.

        Args:
            model_file_path (str): File path of the individual model.
            round_idx (int): Index of the current federated learning round.

        Returns:
            None
        """
        self.test_on_server_for_all_clients_mnn(
            model_file_path, round_idx, report_metrics=False)

    def aggregate(self):
        """
        Aggregate local model updates to obtain the global model.

        Returns:
            averaged_params: Averaged global model parameters.
        """
        logging.info("FedMLDebug. Individual model performance:")
        for idx in range(self.worker_num):
            logging.info("self.model_dict[idx] = {}".format(
                self.model_dict[idx]))
            mnn_file_path = self.model_dict[idx]
            self._test_individual_model_perf_before_agg(
                mnn_file_path, self.args.round_idx)
    ...
    def client_sampling(self, round_idx, client_num_in_total, client_num_per_round):
        """
        Description of the client_sampling method.
        ...
        Returns:
            client_indexes: List of selected client indexes.
        """
    ...
    def _test(self, test_data, device, args):
        """
        Description of the _test method.
    ...
    def test(self, test_data, device, args):
        ...
        Returns:
            Tuple containing test accuracy, test loss, and additional metrics.
        """
        # test data
        test_num_samples = []
        test_tot_corrects = []
    ...
    def test_on_server_for_all_clients_mnn(self, mnn_file_path, round_idx, report_metrics=True):
        ...
        for i in range(self.test_global.iter_number):
            ...
            data = input_data[0]
            # also, model may have more than one outputs
            label = output_target[0]
        ...
        logging.info(
            f"correct = {correct}, self.test_global.size = {self.test_global.size}")
        test_accuracy = correct / self.test_global.size
        test_loss = loss.read()

        if report_metrics:
            logging.info("test acc = {}".format(test_accuracy))
            logging.info("test loss = {}, round loss {}".format(
                test_loss, round(float(np.round(test_loss, 4)), 4)))
            ...
            if self.args.enable_wandb:
                wandb.log(
                    {"round idx": round_idx, "test acc": test_accuracy,
                        "test loss": test_loss, }
                )
