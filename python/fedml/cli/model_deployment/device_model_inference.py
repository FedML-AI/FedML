...
async def predict(request: Request):
    ...
    if in_end_point_id is None or in_end_point_id == "":
        ...
        in_end_point_name = settings.end_point_name
    ...
    model_metrics = FedMLModelMetrics(in_end_point_id, in_end_point_name, in_model_id,
                                      in_model_name,  in_model_version,
    ...
    if auth_request_token(in_end_point_id, in_end_point_name, in_model_name, in_end_point_token):
        ...
        idle_device, model_id, model_name, inference_host, inference_output_url = \
            found_idle_inference_device(in_end_point_id, in_end_point_name,
                                        in_model_id, in_model_name, in_model_version)
        ...
        if inference_output_url != "":
            ...
            inference_response = send_inference_request(idle_device, model_name, inference_host,
                                                        inference_output_url, input_list, output_list)
    ...
    model_metrics.calc_metrics(model_id, model_name, in_end_point_id, in_end_point_name, inference_output_url)

    return inference_response


def found_idle_inference_device(end_point_id, end_point_name, in_model_id, in_model_name, in_model_version):
    idle_device = ""
    model_name = ""
    inference_host = ""
    ...
    payload = FedMLModelCache.get_instance(settings.redis_addr, settings.redis_port).\
        get_idle_device(end_point_id, end_point_name,
                        in_model_id, in_model_name, in_model_version)
...
def send_inference_request(device, model_name, inference_host, inference_url, input_list, output_list):
    try:
