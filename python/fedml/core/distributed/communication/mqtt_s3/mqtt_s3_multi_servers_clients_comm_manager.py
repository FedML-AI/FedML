...
class MqttS3MultiServersClientsCommManager(BaseCommunicationManager):
    ...
    def on_connected(self, mqtt_client_object):
        ...
        if self.args.role == "server":
            ...
            logging.info(f"Multi-server mode, server: {self.server_id} is listening to {self.client_real_ids}, total {self.client_num} clients.")
            for client_rank in range(0, self.client_num):
                real_topic = self._topic + str(self.client_real_ids[client_rank]) + "_" + str(self.server_id)
            ...
            # client
            logging.info(f"Listening to multiple servers: {self.group_server_id_list}")
            for server_id in self.group_server_id_list:
                real_topic = self._topic + str(server_id) + "_" + str(self.client_real_ids[0])
                result, mid = mqtt_client_object.subscribe(real_topic, qos=2)  
    ...
    def send_message(self, msg: Message, wait_for_publish=False):
        ...
        if self.args.role == "server":
            ...
            # client
            topic = self._topic + str(msg.get_sender_id()) + "_" + str(receiver_id)    # send to specific server
