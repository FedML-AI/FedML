...
# from ..mechanisms.binary import Binary

from fedml.core.differential_privacy.common.utils import bernoulli_neg_exp, check_params
...
class PermuteAndFlip(Exponential):
    ...
    def __init__(
        self, *, epsilon, sensitivity, utility, monotonic=False, candidates=None
    ):
        super().__init__(
            epsilon=epsilon,
    ...
    def randomise(self, value=None):
        ...
        raise RuntimeError(
            f"No value to return.  Probabilities: {self._probabilities}."
        )
...
class ExponentialCategorical(DPMechanism):
    ...
    def __init__(self, *, epsilon, utility_list):
        ...
        (
            ...
            self._sensitivity,
            self._domain_values,
        ) = self._build_utility(utility_list)
    ...
    def _get_prob(self, value1, value2):
        ...
        return np.exp(
            -self.epsilon
            * self._get_utility(value1, value2)
            / balancing_factor
            / self._sensitivity
        )
    ...
    def _build_hierarchy(self, nested_list, parent_node=None):
        ...
        for _i, _value in enumerate(nested_list):
            if isinstance(_value, str):
                ...
                raise TypeError(
                    "All leaves of the hierarchy must be a string "
                    + "(see node "
                    + (parent_node + [_i]).__str__()
                    + ")"
                )
            else:
    ...
    def _build_utility_list(hierarchy):
        ...
        for _root_value, _root_hierarchy_locator in hierarchy.items():
            ...
            for _target_value, _target_hierarchy_locator in hierarchy.items():
                ...
                i = 0
                while (
                    i < len(_root_hierarchy_locator)
                    and _root_hierarchy_locator[i] == _target_hierarchy_locator[i]
                ):
                    i += 1
