...
from fedml.core.compression.MLcompression import (
    SPARSIFICATIONS, QUANTIZATIONS, LOWRANK, 
    STATEFUL_COMPRESSORS, check_args_compress, create_compressor)
...
class FedMLCompression:
    ...
    def check_warmup(self, args):
        return getattr(args, "compression_warmup_round", 0) < args.round_idx
    ...
    def compress_named_parameters(self, named_parameters, args, direction="upload"):
        ...
        if self.check_warmup(args):
            logging.info(f"Warming up end, Compressing")
            ...
            logging.info(f"Warming up, not Compressing")
        ...
        if self.check_warmup(args):
            ...
            if compression_name in SPARSIFICATIONS:
                ...
                for k in named_parameters.keys():
                    ...
                    #     model_params[k], model_indexes[k], k
                    # ))
                    named_parameters[k] = self.compressor.decompress(named_parameters[k], k)
            else:
                raise NotImplementedError
        else:
            logging.info(f"Warming up, not Compressing")
