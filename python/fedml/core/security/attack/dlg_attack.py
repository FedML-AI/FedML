...
attack @ client, added by Kai, 07/06/2022
"""


class DLGAttack(BaseAttackMethod):
    def __init__(self, data_size, num_class, model, criterion, attack_epoch, attack_label=84):
        self.data_size = [1] + data_size # batched image size (list), e.g. [1, 3, 32, 32]
    ...
    def attack(self, local_w, global_w, refs=None):
        ...
        print("Ground truth label is %d." % self.attack_label, "Dummy label is %d." % torch.argmax(dummy_label, dim=-1).item())
        ...
        print("Ground truth label is %d." % self.attack_label, "After DLG, Dummy label is %d." % torch.argmax(dummy_label, dim=-1).item())
