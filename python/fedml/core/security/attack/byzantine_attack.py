            ...
            if i not in byzantine_idxs:
                ...
                for k in local_model_params.keys():
                    if is_weight_param(k):
                        local_model_params[k] = torch.from_numpy(np.zeros(local_model_params[k].size())).float().cuda()
    ...
    def _attack_random_mode(self, model_list, byzantine_idxs):
        ...
        for i in range(0, len(model_list)):
            if i not in byzantine_idxs:
                ...
                for k in local_model_params.keys():
                    if is_weight_param(k):
                        local_model_params[k] = torch.from_numpy(np.random.random_sample(size=local_model_params[k].size())).float().cuda()
                new_model_list.append((local_sample_number, local_model_params))
        return new_model_list
