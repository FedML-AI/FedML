...
class InvertAttack(BaseAttackMethod):
    def __init__(
        ...
        loss_fn, _, validloader = inversefed.construct_dataloaders(
            dataset, defs, data_path=data_path
        )
        ...
        if self.dataset == "cifar10":
            self.dm = torch.as_tensor(inversefed.consts.cifar10_mean, **self.setup)[
                :, None, None
            ]
            self.ds = torch.as_tensor(inversefed.consts.cifar10_std, **self.setup)[
                :, None, None
            ]
        ...
        if num_images == 1:
            ...
            if save_files:
                torchvision.utils.save_image(
                    ...
                    f"{self.idx}_{self.arch}_{self.dataset}_input.png",
                )
        else:
            ground_truth, labels = [], []
            idx = 0
            while len(labels) < num_images:
                ...
                if label not in labels:
                    labels.append(
                        torch.as_tensor((label,), device=self.setup["device"])
                    )
    ...
    def attack(self, local_w, global_w, refs=None):
        ...
        test_psnr = inversefed.metrics.psnr(
            output, self.ground_truth, factor=1 / self.ds
        )
        logging.info(
        ...
        if self.save_files and (self.num_images == 1):
            torchvision.utils.save_image(
                output_denormalized, f"{self.idx}_{self.arch}_{self.dataset}_output.png"
            )
