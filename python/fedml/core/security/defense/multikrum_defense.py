...
class MultiKrumDefense(BaseDefenseMethod):
    ...
    def defend(self, local_w, global_w, refs=None):
        ...
        alpha, multikrum_avg ,distance_score = self._multi_krum(_params, self.client_num_per_round, self.byzantine_client_num)
        ...
        for item_index, (k, v) in enumerate(localw0.items()):
            if is_weight_param(k):
                recons_local_w[k] = (
                    torch.from_numpy(multikrum_avg[index_bias : index_bias + v.numel()]).view(v.size())
