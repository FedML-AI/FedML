...
from fedml.core.differential_privacy.fed_privacy_mechanism import DP
...
class DifferentialPrivacy:
    def __init__(self, config):
        ...
        self.dp = DP(config.epsilon, config.delta, config.sensitivity, config.mechanism_type)
    ...
    def run(
        ...
        if self.dp_type == "ldp":
            ...
            for (sample_num, client_grad) in raw_client_grad_list:
                client_grad_list.append((sample_num, self.dp.compute_randomized_gradient(client_grad)))
            # grad_list = [(num, self.dp.randomise(value)) for num, value in raw_client_grad_list]
