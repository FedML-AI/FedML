...
class BulyanDefense(BaseDefenseMethod):
    def __init__(self, byzantine_client_num, client_num_per_round):
        ...
        assert client_num_per_round >= 4 * byzantine_client_num + 3, (
            'users_count>=4*corrupted_count + 3', client_num_per_round, byzantine_client_num)
    ...
    def defend(self, local_w):
        ...
        _len_local_params= vectorize_weight(localw0).shape[0] # lens of the flatted gradients
  
        _params = np.zeros((num_clients, _len_local_params))
        for i in range(num_clients):
            _params[i] = vectorize_weight(local_w[i][1]).cpu().detach().numpy()
        
        select_indexs, selected_set ,agg_grads  = self._bulyan(_params, self.client_num_per_round, self.byzantine_client_num)
        ...
        for item_index, (k, v) in enumerate(localw0.items()):
            if is_weight_param(k):
                recons_local_w[k] = (
                    torch.from_numpy(agg_grads[index_bias : index_bias + v.numel()]).view(v.size())
    ...
    def _bulyan(self, users_params, users_count, corrupted_count):
        ...
        while len(selection_set) < set_size:
            currently_selected = self._krum(users_params, users_count - len(selection_set), corrupted_count, distances, return_index=True)
    ...
    def _krum(self, users_params, users_count, corrupted_count, distances=None, return_index=False):
    ...
    def _krum_create_distances(users_params):
        ...
        for i in range(len(users_params)):
            for j in range(i):
                distances[i][j] = distances[j][i] = np.linalg.norm(users_params[i] - users_params[j])
        return distances
