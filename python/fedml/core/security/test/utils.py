...
def create_fake_model_list_MNIST(active_worker_num):
    ...
    a_local_w["linear.bias"] = torch.FloatTensor(
        [-0.0753, -0.0589, -0.0907, -0.0672, 0.1159, 0.0290]
    )
    ...
    b_local_w["linear.bias"] = torch.FloatTensor(
        [-0.1153, -0.0589, -0.1907, -0.0672, 0.1159, 0.0290]
    )
    ...
    c_local_w["linear.bias"] = torch.FloatTensor(
        [-0.0753, -0.0189, -0.0207, -0.2672, 0.1159, 0.0290]
    )
    model_list = []
    for i in range(active_worker_num):
        if i % 3 == 0:
            model_list.append(
                (20 + i, copy.deepcopy(a_local_w))
            )  # add a random sample num
        if i % 3 == 1:
            model_list.append(
                (30 + i, copy.deepcopy(b_local_w))
            )  # add a random sample num
        if i % 3 == 2:
            model_list.append(
                (25 + i, copy.deepcopy(c_local_w))
            )  # add a random sample num
    return model_list
...
def normal_aggregation(client_grad_list):
