...
series = read_csv('data/qps_test_exponential_distribution.csv', header=0, index_col=0, parse_dates=True, date_parser=parser)
...
plt.scatter([i for i in range(len(series.values))][:train_size], series.values[:train_size], color='green', label = 'Train')
...
plt.grid(True)
plt.yscale("log")
plt.savefig('plot/sample_arima_exponential_p{}_d{}_q{}.png'.format(p, d, q))
