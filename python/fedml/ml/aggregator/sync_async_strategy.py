...
class BaseAggStrategy(ABC):
        ...
        if self.is_async:
            ...
            if hasattr(config, "buffer_size") and isinstance(config.buffer_size, int):
                if config.buffer_size >= config.client_num_per_round or config.buffer_size <= 0:
                    raise AttributeError(f"config.buffer_size ({config.buffer_size}) is abnormal.")
                else:
            ...
            if hasattr(config, "max_acceptable_staleness") and isinstance(config.max_acceptable_staleness, int): 
                if config.max_acceptable_staleness < 0:
                    raise AttributeError(f"config.max_acceptable_staleness ({config.max_acceptable_staleness}) is abnormal.")
                else:
    ...
    @abstractmethod
    def whether_to_accept(self, current_global_agg_step, current_client_agg_step):
        pass
    ...
    @abstractmethod
    def get_weight_scaling_ratio(self, current_global_agg_step, current_client_agg_step):
        pass
...
class FedBuffAsyncAggStrategy(BaseAggStrategy):
    ...
    def whether_to_accept(self, current_global_agg_step, current_client_agg_step):
        staleness = current_global_agg_step - current_client_agg_step
