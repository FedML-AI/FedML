...
from .constants.prompt_template import ARCHITECTURE_TO_PROMPT_TEMPLATE
from .modeling_utils import get_max_seq_length
from .typing import ModelConfigType, PathType, PipelineType
from .utils import get_real_path, is_directory, is_file
...
class ModelArguments:
    ...
    max_history_tokens: int = field(
        default=-1,
        metadata={
            "help": "Number of tokens for conversation history. Set to 0 to disable history. Set to a negative"
                    " value to automatically infer it.",
        }
    )
    prompt_style: str = field(
        default="auto",
        metadata={
            "help": "Prompt template style. Set to \"auto\" to automatically infer it.",
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_history_tokens < 0:
            # auto infer
            if model_context_length is not None:
                self.max_history_tokens = model_context_length // 4
            else:
                self.max_history_tokens = self.max_new_tokens // 2

        if self.prompt_style == "auto":
            # infer prompt style
            if is_directory(self.model_name_or_path):
                warnings.warn(
                    f"Inferring prompt template type from local model. This may be inaccurate."
                    f" For the best output quality, use `--prompt_style` to specify prompt type."
                )

                config: ModelConfigType = AutoConfig.from_pretrained(self.model_name_or_path)
                architecture = config.architectures[0]

                if architecture in ARCHITECTURE_TO_PROMPT_TEMPLATE:
                    self.prompt_style = ARCHITECTURE_TO_PROMPT_TEMPLATE[architecture]
                else:
                    warnings.warn(
                        f"Cannot find suitable prompt style for \"{self.model_name_or_path}\","
                        f" use default prompt style."
                    )
                    self.prompt_style = "default"

            elif self.model_name_or_path.startswith(("meta-llama/Llama-2-", "mistralai/Mistral-")):
                self.prompt_style = "llama"

            elif self.model_name_or_path.startswith(("EleutherAI/pythia-", "databricks/dolly-")):
                self.prompt_style = "dolly"

            elif self.model_name_or_path.startswith(("OpenAssistant/llama2-13b-orca-", "OpenAssistant/oasst-sft-")):
                self.prompt_style = "oasst"

            elif self.model_name_or_path.startswith(("HuggingFaceH4/mistral-", "HuggingFaceH4/zephyr-")):
                self.prompt_style = "zephyr"

            else:
                warnings.warn(
                    f"Cannot find suitable prompt style for \"{self.model_name_or_path}\", use default prompt style."
                )
                self.prompt_style = "default"
    ...
    @property
    def prompt_info(self) -> PromptInfo:
        return PROMPT_TEMPLATE_MAPPING[self.prompt_style]
    ...
    def model_kwargs(self) -> Dict[str, Any]:
        ...
        if self.model_dtype in ("fp32", "fp16", "bf16"):
            return dict(torch_dtype=torch_dtype)
