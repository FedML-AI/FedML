...
class UnitedLLMTrainerCallback(TrainerCallback):
...
class UnitedLLMTrainer(HFTrainer):
    def __init__(
            self,
            ...
            resume_train_callback: Optional[UnitedLLMTrainerCallback] = None
    ):
        super().__init__(
            model=model,
            args=args,
        ...
        if resume_train_callback is None:
            resume_train_callback = UnitedLLMTrainerCallback()
    ...
    def add_callback(self, callback: Union[Type[TrainerCallback], TrainerCallback]) -> None:
        ...
        if issubclass(cb_class, UnitedLLMTrainerCallback):
            # at most one `UnitedLLMTrainerCallback` is allowed
            if not self.has_callback(cb) and self.has_callback(UnitedLLMTrainerCallback):
                self.pop_callback(UnitedLLMTrainerCallback)
