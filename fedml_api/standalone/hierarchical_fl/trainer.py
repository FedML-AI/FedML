...
class Trainer(FedAvgTrainer):
    ...
    def train(self):
        ...
        for global_round_idx in range(self.args.global_comm_round):
            ...
            w_groups_list = [[] for _ in range(self.args.group_comm_round*self.args.epochs)]
            ...
            w_global_list = []
            for w_groups in w_groups_list:
                w_global_list.append(self.aggregate(w_groups))

            # update last weight
            w_global = w_global_list[-1]

            # evaluate performance for all epochs
            for epoch, w_global in enumerate(w_global_list):
                global_epoch = global_round_idx*self.args.group_comm_round*self.args.epochs+epoch
