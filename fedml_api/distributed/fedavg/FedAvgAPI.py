...
from fedml_api.distributed.fedavg.FedAvgClientManager import FedAVGClientMananger
from fedml_api.distributed.fedavg.FedAvgServerManager import FedAVGServerMananger
...
def init_server(args, device, comm, rank, size, model, train_data_num, train_data_global, test_data_global):
    ...
    server_manager = FedAVGServerMananger(args, comm, rank, size, aggregator)
    server_manager.run()
...
def init_client(args, device, comm, process_id, size, model, train_data_num, local_data_num, train_data_local):
    ...
    client_manager = FedAVGClientMananger(args, comm, process_id, size, trainer)
    client_manager.run()
