...
class SysStats:
    ...
    def produce_info(self):
        ...
        if self.sys_stats_impl.gpu_count >= 1:
            self.gpu_utilization = round(gpu_usage_total / self.sys_stats_impl.gpu_count, 2)
            self.gpu_memory_allocated = round(gpu_mem_allocated / self.sys_stats_impl.gpu_count)
            self.gpu_temp = round(gpu_temperature_total / self.sys_stats_impl.gpu_count)
            self.gpu_power_usage = round(gpu_power_usage_total / self.sys_stats_impl.gpu_count)
            self.gpu_time_spent_accessing_memory = round(gpu_mem_used / self.sys_stats_impl.gpu_count)
    ...
    def produce_info_internal(self):
        ...
        self.process_memory_available = total_mem_size * (1.0 - process_mem_percent_total)
        ...
        self.network_traffic = net_io_counters.bytes_sent - self.network_init["bytes_send"] +\
                               net_io_counters.bytes_recv - self.network_init["bytes_recv"]
        ...
        for i in range(gpu_device_count):
            ...
            gpu_temperature_total += pynvml.nvmlDeviceGetTemperature(gpu_handle, NVML_TEMPERATURE_GPU)
