...
export class MLOpsStore {
  ...
  mlops_metrics: any;
  mlops_event!: {
    log_event_started: (arg0: any, arg1: any, arg2: any) => void;
    log_event_ended: (arg0: any, arg1: any, arg2: any) => void;
  };
  ...
  event(event_name: any, event_started = true, event_value: any, event_edge_id: null) {
    if (!this.mlops_enabled(this.mlops_args)) return;

    this.set_realtime_params();

    // set_log_mqtt_mgr();

    if (!this.mlops_bind_result) return;

    if (event_started) this.mlops_event.log_event_started(event_name, event_value, event_edge_id);
    else this.mlops_event.log_event_ended(event_name, event_value, event_edge_id);
  }
