...
export class Arguments<T extends RunArgsJSON> {
  ...
  async get_default_yaml_config(cmd_args: T, training_type = null, comm_backend = null) {
  ...
  set_attr_from_config(configuration: { [x: string]: { [x: string]: any } }) {
    for (const i in configuration) {
      if (typeof configuration[i] == 'object')
        for (const j in configuration[i]) this[j] = configuration[i][j];
    }
  }
