...
export async function exportCifar10BatchData(args, client_id, trainBatchSize, testBatchSize) {
  ...
  const trainDeviceData = trainXs.slice([trainDataLength * (client_idx - 1), 0, 0, 0], [trainDataLength, 32, 32, 3])
  const trainDeviceLabel = trainYs.slice([trainDataLength * (client_idx - 1), 0], [trainDataLength, 10])
  const testDeviceData = testXs.slice([testDataLength * (client_idx - 1), 0, 0, 0], [testDataLength, 32, 32, 3])
  const testDeviceLabel = testYs.slice([testDataLength * (client_idx - 1), 0], [testDataLength, 10])
