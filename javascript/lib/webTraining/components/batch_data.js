...
export async function exportBatchData(args, client_id, trainBatchSize, testBatchSize) {
  ...
  // split the dataSet
  const trainDeviceData = trainXs.slice([trainDataLength * (client_idx - 1), 0, 0, 0], [trainDataLength, 28, 28, 1])
  const trainDeviceLabel = trainYs.slice([trainDataLength * (client_idx - 1), 0], [trainDataLength, 10])
  const testDeviceData = testXs.slice([testDataLength * (client_idx - 1), 0, 0, 0], [testDataLength, 28, 28, 1])
