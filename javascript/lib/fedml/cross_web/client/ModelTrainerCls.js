...
export class ModelTrainerCLS {
  ...
  async train() {
    ...
    if (this.args.dataset == 'mnist') {
      await model.fit(tf.reshape(this.trainData, [this.trainData.shape[0], -1]), this.trainDataLabel, {
        validationData: [
  ...
  get_model_params() {
    try {
      ...
      for (let i = 0; i < this.model.getWeights().length; i++) {
        dict = { model: this.model.getWeights()[i], params: this.model.getWeights()[i].dataSync() }
        out.push(dict)
      }
  ...
  set_model_params(model_parameters, verbose = false) {
    ...
    if (this.model.getWeights().length == model_parameters.length) {
      ...
      if (verbose)
        ...
        console.log('ERROR: The model structure of pytorch and tensorflow.js is not aligned! Cannot transfer parameters accordingly.')
    }
  }
}
