...
export class ClientMasterManager extends FedMLCommManager {
  ...
  register_message_receive_handlers() {
    ...
    super.register_message_receive_handler(MyMessage.MSG_TYPE_CONNECTION_IS_READY, this.handle_message_connection_ready.bind(this))
    super.register_message_receive_handler(MyMessage.MSG_TYPE_S2C_CHECK_CLIENT_STATUS, this.handle_message_check_status.bind(this))
    super.register_message_receive_handler(MyMessage.MSG_TYPE_S2C_INIT_CONFIG, this.handle_message_init.bind(this))
    super.register_message_receive_handler(MyMessage.MSG_TYPE_S2C_SYNC_MODEL_TO_CLIENT, this.handle_message_receive_model_from_server.bind(this))
    super.register_message_receive_handler(MyMessage.MSG_TYPE_S2C_FINISH, this.handle_message_finish.bind(this))
  }
  ...
  async handle_message_init(msg) {
    ...
    this.report_client_training_status(this.args.client_id_list[0], this.args.run_id, MyMessage.MSG_MLOPS_CLIENT_STATUS_TRAINING)
  ...
  async handle_message_receive_model_from_server(msg) {
    ...
    if (this.round_idx < this.num_rounds) {
      ...
      this.report_client_training_status(this.args.client_id_list[0], this.args.run_id, MyMessage.MSG_MLOPS_CLIENT_STATUS_FINISHED)
      super.finish()
    }
  }
  ...
  cleanup() {
    ...
    this.report_client_training_status(this.args.client_id_list[0], this.args.run_id, MyMessage.MSG_MLOPS_SERVER_STATUS_FINISHED)
    super.finish()
  ...
  report_client_device_status_to_web_ui(edge_id, runId, status) {
    ...
    this.com_manager.mqtt_mgr.publish('fl_client/mlops/status', JSON.stringify(payload), { qos: 2, retain: true }, (error) => {
      if (error)
        console.log(error)
      else
  ...
  common_report_client_id_status(edge_id, runId, status) {
    ...
    this.com_manager.mqtt_mgr.publish(`fl_client/flclient_agent_${edge_id}/status`, JSON.stringify(payload), { qos: 2, retain: true }, (error) => {
      if (error)
        console.log(error)
      else
  ...
  common_report_client_training_status(edge_id, runId, status) {
    ...
    this.com_manager.mqtt_mgr.publish('fl_run/fl_client/mlops/status', JSON.stringify(payload), { qos: 2, retain: true }, (error) => {
      if (error)
        console.log(error)
      else
  ...
  async send_model_to_server(receive_id, weights) {
    ...
    const message = new Message(MyMessage.MSG_TYPE_C2S_SEND_MODEL_TO_SERVER, this.client_real_id, receive_id)
  ...
  async send_client_status(receive_id, status = 'ONLINE') {
    ...
    const message = new Message(MyMessage.MSG_TYPE_C2S_CLIENT_STATUS, this.client_real_id, receive_id)
  ...
  async log_client_model_info(round_index, model_url) {
    ...
    this.com_manager.mqtt_mgr.publish('fl_server/mlops/client_model', message_json, { qos: 2, retain: true }, (error) => {
      if (error)
        console.log(error)
      else
