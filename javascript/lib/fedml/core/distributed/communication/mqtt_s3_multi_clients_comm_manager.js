...
export class MqttS3MultiClientsCommManager {
  constructor(config_path, s3_config_path, topic_in = 'fedml', client_rank = 0, client_num = 0, args) {
      ...
      if (topic
        ...
        if (s3_key_str != undefined) {
          ...
          py_layers.forEach((item) => {
            newWeights.push(tf.transpose(tf.tensor(json[item])))
          })
          console.log('get the server model weights: ', newWeights)
          const model_params = newWeights
          console.log('mqtt_s3.on_message: receive model_params: ', model_params)
  ...
  async send_message(msg) {
    ...
    if (model_params_object !== undefined) {
      console.log('mqtt_s3.send_message: S3+MQTT msg sent, message_key: ', message_key)
      const model_url = await this.s3_storage.write_model(message_key, model_params_object)
    ...
    setTimeout(() => {
      ...
      this.mqtt_mgr.publish(topic, JSON.stringify(payload), { qos: 2, retain: true }, (error) => {
        if (error)
          console.log(error)
