...
RESULTS_DIR = '/data/home/lvzx/FedML/python/examples/federate/prebuilt_jobs/shieldfl/results'

def load_exp(model, dataset, alpha, seed):
    pattern = f'{RESULTS_DIR}/metrics_{model}_{dataset}_shieldfl_atknone_defnone_a{alpha}_pmr0.0_gauto_seed{seed}.jsonl'
    files = glob.glob(pattern)
    if not files:
        return None
    data = [json.loads(l) for l in open(files[0])]
    return data
